...
const escapeCodeDefault = {
  ...
  STATUS_PAPER_ROLL: Buffer.from([0x10, 0x04, 0x04]), // Paper roll status
  STATUS_PRINT:      Buffer.from([0x10, 0x04, 0x11]), // Print status
  STATUS_FULL:       Buffer.from([0x10, 0x04, 0x14]), // Full status
  ...
  HW_SELECT:       Buffer.from([0x1b, 0x3d, 0x01]), // printing select
  HW_RESET:        Buffer.from([0x1b, 0x3f, 0x0a, 0x00]), // Reset printing hardware
  ...
  // All code pages supported by printing.
...
class EpsonDriver extends Driver {
  ...
  getErrorStatus(status) {
    ...
    return {
      cutterError:          !!(status & 8),
      unrecoverableError:   !!(status & 32),
      autoRecoverableError: !!(status & 64),
    };
  }

  getPaperRollStatus(status) {
    status = this.convertStatus(status);

    return {
      paperLow:        !!(status & 4), // ou 8
      paperNotPresent: !!(status & 32), // ou 64
    };
  }
  ...
  convertStatus(status) {
    return parseInt(status.toString("hex"), 16);
  }
}

module.exports = { EpsonDriver };
